...
def summarize_ragas_scores(rows: list[dict[str, Any]], answer_rows: list[dict[str, Any]] | None = None) -> dict[str, Any]:
    ...
    if answer_rows is not None:
        ...
        summary["retrieval_failure_rate"] = (degraded + failed) / total if total else None
    return summary
...
def save_ragas_placeholder(base_dir: Path, answer_rows: list[dict[str, Any]], error: str) -> None:
    ...
    atomic_write_json(base_dir / "summary.json", summarize_ragas_scores(ensure_rows, answer_rows=answer_rows))


def save_ragas_outputs(base_dir: Path, rows: list[dict[str, Any]], answer_rows: list[dict[str, Any]] | None = None) -> None:
    ...
    atomic_write_json(base_dir / "summary.json", summarize_ragas_scores(rows, answer_rows=answer_rows))
